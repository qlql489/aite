...
use objc2::{
  define_class, ffi, msg_send, rc::Retained, runtime::NSObject, ClassType, DeclaredClass,
...
pub(crate) trait WebViewUIDelegateClass: DeclaredClass<Ivars = WryWebViewUIDelegateIvars> {}
...
impl WebViewUIDelegateClass for WryWebViewUIDelegate {}

define_class!(
  #[unsafe(super(NSObject))]
  #[name = "WryWebViewUIMediaCaptureDelegate"]
  #[thread_kind = MainThreadOnly]
  #[ivars = WryWebViewUIDelegateIvars]
  pub struct WryWebViewUIMediaCaptureDelegate;

  unsafe impl NSObjectProtocol for WryWebViewUIMediaCaptureDelegate {}

  unsafe impl WKUIDelegate for WryWebViewUIMediaCaptureDelegate {
    #[cfg(target_os = "macos")]
    #[unsafe(method(webView:runOpenPanelWithParameters:initiatedByFrame:completionHandler:))]
    fn run_file_upload_panel(
      &self,
...
impl WebViewUIDelegateClass for WryWebViewUIMediaCaptureDelegate {}

fn protocol_has_instance_method<P: ?Sized + ProtocolType>(selector: objc2::runtime::Sel) -> bool {
  let Some(protocol) = P::protocol() else {
    return false;
  };

  let description =
    unsafe { ffi::protocol_getMethodDescription(protocol, selector, false.into(), true.into()) };
  description.name.is_some()
}

fn new_ui_delegate<T: WebViewUIDelegateClass>(
...
where
  <T as ClassType>::Super: ClassType,
{
  #[cfg(target_os = "ios")]
...
impl WryWebViewUIDelegate {
  pub fn supports_media_capture_permission() -> bool {
    protocol_has_instance_method::<dyn WKUIDelegate>(
  ...
  pub fn new(
    ...
    new_ui_delegate(mtm, new_window_req_handler)
  }
}

impl WryWebViewUIMediaCaptureDelegate {
  pub fn new(
    mtm: MainThreadMarker,
    new_window_req_handler: Option<
      Box<dyn Fn(String, NewWindowFeatures) -> NewWindowResponse + Send + Sync>,
