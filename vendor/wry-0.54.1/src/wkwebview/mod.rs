...
use class::{
  ...
  wry_navigation_delegate::{WryNavigationDelegate, WryNavigationDownloadDelegate},
  wry_web_view::WryWebViewIvars,
  wry_web_view_delegate::{WryWebViewDelegate, IPC_MESSAGE_HANDLER_NAME},
  wry_web_view_ui_delegate::{WryWebViewUIDelegate, WryWebViewUIMediaCaptureDelegate},
};
...
pub(crate) struct InnerWebView {
  ...
  #[allow(dead_code)]
  // We need this the keep the reference count
  navigation_policy_delegate: Retained<NSObject>,
  #[allow(dead_code)]
  // We need this the keep the reference count
  download_delegate: Option<Retained<WryDownloadDelegate>>,
  #[allow(dead_code)]
  // We need this the keep the reference count
  ui_delegate: Retained<NSObject>,
  #[cfg(target_os = "macos")]
...
impl InnerWebView {
  ...
  fn new_ns_view(
    ...
    unsafe {
      ...
      let supports_downloads = objc2::available!(ios = 14.5, macos = 11.3, ..);
      let has_download_handler =
        supports_downloads && attributes.download_started_handler.is_some();
      // Download handler
      ...
      let navigation_policy_delegate = if has_download_handler {
        let delegate = WryNavigationDownloadDelegate::new(
          webview.clone(),
        ...
        let proto_navigation_policy_delegate = ProtocolObject::from_ref(&*delegate);
        webview.setNavigationDelegate(Some(proto_navigation_policy_delegate));
        delegate.into_super()
      } else {
        let delegate = WryNavigationDelegate::new(
          webview.clone(),
          pending_scripts.clone(),
          has_download_handler,
          attributes.navigation_handler,
      ...
      let ui_delegate = if WryWebViewUIDelegate::supports_media_capture_permission() {
        let delegate =
          WryWebViewUIMediaCaptureDelegate::new(mtm, attributes.new_window_req_handler);
        let proto_ui_delegate = ProtocolObject::from_ref(&*delegate);
        webview.setUIDelegate(Some(proto_ui_delegate));
        delegate.into_super()
      } else {
        let delegate = WryWebViewUIDelegate::new(mtm, attributes.new_window_req_handler);
        let proto_ui_delegate = ProtocolObject::from_ref(&*delegate);
        webview.setUIDelegate(Some(proto_ui_delegate));
        delegate.into_super()
      };
