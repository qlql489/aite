...
use super::class::wry_navigation_delegate::NavigationDelegateClass;

pub(crate) fn did_commit_navigation<T: NavigationDelegateClass>(
  this: &T,
  webview: &WKWebView,
...
pub(crate) fn did_finish_navigation<T: NavigationDelegateClass>(
  this: &T,
  _webview: &WKWebView,
  _navigation: &WKNavigation,
) {
...
// Navigation handler
pub(crate) fn navigation_policy<T: NavigationDelegateClass>(
  this: &T,
  _webview: &WKWebView,
  ...
  unsafe {
    ...
    if supports_downloads && should_download {
...
// Navigation handler
pub(crate) fn navigation_policy_response<T: NavigationDelegateClass>(
  this: &T,
  _webview: &WKWebView,
  ...
  unsafe {
    ...
    if supports_downloads && !can_show_mime_type {
      let has_download_handler = this.ivars().has_download_handler;
      if has_download_handler {
...
pub(crate) fn web_content_process_did_terminate<T: NavigationDelegateClass>(
  this: &T,
  _webview: &WKWebView,
) {
