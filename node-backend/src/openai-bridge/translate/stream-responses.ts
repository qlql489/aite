...
import type { AnthropicStreamEvent, AnthropicResponse } from '../types/anthropic.js';
import type { ResponsesStreamEvent, ResponsesUsage } from '../types/openai-responses.js';
import { generateMessageId, generateToolUseId } from '../utils/id.js';
import { emptyUsage, fromResponsesUsage, toAnthropicUsage, type UsageSnapshot } from './usage.js';
...
export class ResponsesStreamTranslator {
  ...
  private handleFailed(
    ...
    if (response.usage) {
      this.usage = fromResponsesUsage(response.usage);
    }
