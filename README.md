...
### 🔄 多项目与会话管理
把散落在不同终端里的 Claude Code 工作上下文收拢到同一个桌面工作台
- 本地项目快速导入与切换
- 会话创建、恢复、持久化与继续对话
- 恢复历史消息顺序，以及模型、权限模式、思考强度等会话配置
- 历史会话搜索与快速回溯，便于多任务并行推进

### 💬 消息与工具可视化
让 Claude Code 的输出、中间过程和执行状态都清晰可见
- 流式展示 thinking、工具调用、子代理、图片、附件等全类型消息
- 清晰区分运行中、已完成、失败、等待审批等执行状态
- 工具调用结构化呈现，Edit/Write 支持 diff 视图
- 子代理执行过程独立展示，便于查看步骤、状态与结果
- 权限请求在聊天流中直接审批，支持批准/拒绝/始终允许，并可切换会话级权限模式
- 待办事项可视化展示进度与完成状态

### 📁 文件与代码协同
把聊天、代码现场和编辑器上下文放到同一个工作区里
- `@` 文件引用，支持目录、图片和附件上传
- 工作区目录树浏览、项目内文件搜索与内联轻量编辑
- Git 状态、分支与变更信息集成
- 检测并连接 IDE，可将当前选中代码带入对话上下文

### ⚙️ 配置与扩展管理
把原本分散在 CLI、配置文件和环境变量里的能力集中到图形界面
- 全局与项目级 MCP Server、skills、commands 统一管理
- 模型切换、思考强度与默认权限模式可视化配置
- 支持接入自定义大模型，兼容 Anthropic / OpenAI 协议并支持 provider bridge
- 按模型、会话、时间线查看 Token usage 与成本统计
- 支持 Claude CLI 参数保存、安装检测与自动更新
